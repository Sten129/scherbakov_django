...
from backend.models import Type, Genre, Technic, Document, Article, Letter, Location, Persone, Picture, Owner, \
    Exhibition
...
class TestGenreAPI:
    ...
    def test_genre_not_found(self, client):
        response = client.get(f'/api/genres')
        assert response.status_code != 404, ('/api/genres')
